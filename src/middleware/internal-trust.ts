...
// ─────────────────────────────────────────────────────────────────────────────
// CIDR matching (IPv4 + IPv6) — minimal, no external deps
// ─────────────────────────────────────────────────────────────────────────────
...
function parseCidr(spec: string): Cidr | null {
  ...
  if (ip.includes(':')) {
    ...
    if (network == null || bits > 128) return null;
...
export function parseCidrList(csv: string): Cidr[] {
  return csv
    .split(',')
    .map((s) => parseCidr(s))
...
export function isAddressInCidrList(addr: string | undefined, cidrs: Cidr[]): boolean {
  if (!addr || cidrs.length === 0) return false;
 ...
 */
export function verifyInternalAuthHmac(
  secret: string,
  userId: string,
  provided: string,
): boolean {
